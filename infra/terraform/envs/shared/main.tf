...
# MLflow artifact store (ADR-0032, F3-08). Backend store is Postgres in the
# cloud (Supabase/Neon), out of Terraform's scope.
# create_iam_user=false: the org SCP (p-rujzp9jo) explicitly denies
# iam:CreateUser in this sandbox account, so team members reach the bucket with
# their AWS SSO credentials instead of a dedicated static key.
module "s3_mlflow" {
